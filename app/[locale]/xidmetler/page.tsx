...
export default function ServicesPage() {
  ...
  const title = cleanText(servicesSection?.title) || t("services.title");
      ...
      : [
          {
            ...
            buttonText: t("servicesPage.moreDetails"),
          },
          {
            title: "Qaynaq Sexi",
  ...
  const bullets = [
    t("servicesPage.customDesign"),
    t("servicesPage.qualityControl"),
    t("servicesPage.fastExecution"),
    t("servicesPage.guaranteedWork"),
  ];
              ...
              <div className="rounded-3xl border border-black/10 bg-white p-7 shadow-[0_15px_40px_rgba(0,0,0,0.08)]">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-black">{t("servicesPage.selectService")}</div>
                    <div className="mt-2 text-[13px] leading-6 text-black/70">
                      {t("servicesPage.selectServiceDesc")}
                    </div>
                  </div>
                </div>
      ...
      <section className="py-14 md:py-18">
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex items-end justify-between gap-6">
            <div>
              <h2 className="text-3xl font-extrabold text-black">{t("servicesPage.sections")}</h2>
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {list.map((it: any, idx: number) => {
              const itTitle = cleanText(it?.title) || `${t("serviceDetail.service")} ${idx + 1}`;
              ...
              const btn = cleanText(it?.buttonText) || t("servicesPage.moreDetails");
              ...
              return (
                ...
                >
                  <div className="relative h-52 w-full">
                    <Image src={img} alt={itTitle} fill className="object-cover" />
                  ...
                  <div className="p-6">
                    <div className="text-xl font-extrabold text-black">{itTitle}</div>
                    <p className="mt-2 text-[14px] leading-7 text-black/70">
                      {d || t("servicesPage.detailsFallback")}
                    </p>
          ...
          <div className="mt-10 rounded-3xl border border-black/10 bg-[#F7F7F7] p-6 text-sm text-black/70">
            * {t("servicesPage.orderNote")}{" "}
            <Link href={`/${locale}/elaqe`} className="font-semibold text-black underline underline-offset-4">
              {t("servicesPage.contactPage")}
            </Link>{" "}
            {t("servicesPage.visitText")}
          </div>
        </div>
      </section>
    </main>
